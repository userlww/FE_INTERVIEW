相比直接使用原生JavaScript操作DOM来完成页面交互，Vue最大的优势就在于它的数据驱动视图，也就是说当数据发生变化时，视图能够自动进行对应的更新，而不需要开发者在页面脚本中直接操作DOM，这使得前端开发的体验有了质的飞跃，那么，数据的变化是如何通知Vue的呢，这就是本文要讲述Vue数据响应式原理。
